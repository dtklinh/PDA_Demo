...
filter_by_low_abundance <- function(phyloseqObj, threshold, A){
  ...
  physeq.abun.filt <- prune_taxa(taxa = noCont,x = phyloseqObj)
...
## we dont remove samples??
visualize_samples_low_reads <- function(PhyObj){
...
filter_samples_low_reads <- function(PhyObj, threshold){
  ...
  physeq.read.filt <- prune_taxa(taxa_sums(PhyObj)>0, PhyObj)
}
##------------------------------------------
