...
p.true.original.1500.80 <- merge_phyloseq(physeq1.1500.80,physeq2.1500.80,physeq3.1500.80,physeq4.1500.80,physeq6.1500.80,physeq7.1500.80)#merge the different phyloseq objects into 1 phyloseq object
...
alpha_analysis(p.comb.filt.s.1500.80,i = "TRUE_control")
beta_analysis_bray(p.comb.filt.s.1500.80,i = "TRUE_control")
```
...
for (i in IDs) {
  b <- subset_samples(p.comb, DNAex_round == i)
  ...
  if (check3 != 0) {
    ...
    df.filt$pval <- vec
    bad_taxa <- list.append(bad_taxa,df.filt[df.filt$pval > 0.05,"OTU"])
    y <- paste0("df.filt1-buffer-",i)
    assign(y,df.filt)
  }
  if (check4 != 0) {
    ...
    # Im honest: this is a bit redundant
    df.filt <- df.filt <- data.frame(OTU = taxa_names(b.true),
                          x = prevalence(b.true, detection = 0, count = TRUE),
                          n = nsamples(b.true),
                          p = prevalence(b.condition, detection = 0, count = FALSE)) #df stores x,n,p for binom.test
    vec <- seq(1,nrow(df.filt))
    ...
    df.filt$pval <- vec
    bad_taxa <- list.append(bad_taxa,df.filt[df.filt$pval > 0.05,"OTU"])
    y <- paste0("df.filt1-PCR_ctrl_H2O-",i)
    assign(y,df.filt)
  }
...
for (i in IDs) {
  b <- subset_samples(p.comb, PCR_round == i)
  b <- prune_taxa(taxa = (taxa_names(b)[-which(taxa_sums(b) == 0)]),x = b) #this removes taxa with taxa_sums == 0 from the overall batch
  check1 <- length(which(meta(b)$TRUE_control == "TRUE")) #checks whether the batch contains true_samples
  check2 <- length(which(meta(b)$TRUE_control == "control")) #checks whether the batch contains nct_sample
  check3 <- length(which(meta(b)$sample_side == "buffer")) #checks whether the batch contains buffer_nct_samples
  ...
  if (check4 != 0) {
    ...
    df.filt$pval <- vec
    bad_taxa <- list.append(bad_taxa,df.filt[df.filt$pval > 0.05,"OTU"])
    y <- paste0("df.filt2-PCR_ctrl_H2O-",i)
    assign(y,df.filt)
  }
  n_paraffin <- (check5+check6)
  if (n_paraffin != 0) {
...
Further, we have a look at those *`r length(intersect(p.true.RmLowAbun.RmHLPrev.man %>% otu_table() %>% rownames(),physeq.filt.nct.s.1500.80.unify %>% otu_table() %>% rownames()))`* taxas in NCTs but not contaminants
...
In addition we provide the filtering step using a relaxed setting version of the binomial exact test. In this version - given a batch consisting of (n) true samples and (v) negative controls sequenced alongside them  - we compare the prevalence in true samples (x) of an OTU with the probability of presence derived from the negative controls (p). We do not breakdown the negative controls into further sub-categories ("paraffin","buffer","PCR_H2O").

### Binomial exact test - relaxed
...
Rm_taxa <- list()
# for DNAex
IDv <- sample_data(p.comb)$DNAex_round %>% unique()
for (i in IDv) {
  b <- subset_samples(p.comb, DNAex_round == i)
...
# for PCR
IDv <- sample_data(p.comb)$PCR_round %>% unique()
for (i in IDv) {
  b <- subset_samples(p.comb, PCR_round == i)
  check.true <- length(which(meta(b)$TRUE_control == "TRUE"))
  check.nct <- length(which(meta(b)$TRUE_control == "control"))
...
p.comb <- subset_taxa(p.comb, !(taxa_names(p.comb) %in% Rm_taxa))
p.true.RmLowAbun.RmHLPrev.man <- subset_samples(physeq = p.comb, TRUE_control == "TRUE")
p.true.RmLowAbun.RmHLPrev.man <- prune_taxa(x = p.true.RmLowAbun.RmHLPrev.man,taxa = taxa_names(p.true.RmLowAbun.RmHLPrev.man)[-(which(taxa_sums(p.true.RmLowAbun.RmHLPrev.man)==0))])
p.true.RmLowAbun.RmHLPrev.man
```
...
p.true.RmLowAbun.RmHLPrev.batchDNA.man
rm(batch_type)
rm(otu)
rm(sub.meta.data)
rm(batchid)
...
p.true.RmLowAbun.RmHLPrev.batchPCR.man
rm(batch_type)
rm(otu)
rm(sub.meta.data)
rm(batchid)
