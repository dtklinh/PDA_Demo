...
```{r}
p.true.RmLowAbun <- filter_by_low_abundance(phyloseqObj = p.true.filt.s,threshold =0.0001 ,A = 1)
```
...
PlotVenn2Sets(taxa_names(p.true.RmLowAbun) %>% unique(),
              taxa_names(physeq.nct.filt.s) %>% unique(),
              "True Samples" , "NCT Samples ")
```


There are `r setdiff(taxa_names(physeq.nct.filt.s), taxa_names(p.true.RmLowAbun)) %>% length()` taxa belong to NCT samples but not in true samples (after rule of thumb). They are obvious contamination.
...
p.true.RmLowAbun.RmHLPrev <- Wrapper_Nejman(p.true.RmLowAbun.RmHighPrev,physeq.nct.filt.s,metadata,x = p)
```
