...
rm(physeq.nct.filt)
```
**Further, Remove low read count NCTs**
...
We now remove those taxa which have high prevalence in NTC samples out of true (and NTC) samples.
```{r}
...
```
Now we have `r nrow(sample_data(p.true.RmLowAbun.RmHighPrev))` true samples and `r nrow(otu_table(p.true.RmLowAbun.RmHighPrev))` taxa. In NCTs, we have `r nrow(sample_data(physeq.nct.filt.s.prune))` control samples and `r nrow(otu_table(physeq.nct.filt.s.prune))` taxa
...
physeq.comb.filt <- merge_phyloseq(p.true.RmLowAbun.RmHighPrev, physeq.nct.filt.s.prune)
...
Plot_PCoA(TAX=taxa_corrected2, factor=batchid, main="ConQuR (Default), Bray-Curtis - DNAex & PCR_round")
...
Plot_PCoA(TAX=taxa_corrected2, factor=batchid, dissimilarity="Aitch", main="ConQuR (Default), Aitchison - DNAex & PCR_round")
```
...
compositional_analysis(p.true.original, target_rank = "species", rel_abundance = 0.03)
```
